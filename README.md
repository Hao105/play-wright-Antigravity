...
### 增強 AI 能力：載入 Playwright Skills
光有 MCP 是不夠的，AI 預設生成的測試常因為缺少自動化框架語境而變得脆弱（例如誤用 CSS Selectors 或是沒有適當的等待機制）。
載入 Playwright Skills 能夠強制 AI 遵守企業級的開發規範：
```powershell
# 安裝涵蓋 POM、定位器與 CI 最佳實踐的技能包
npx skills add testdino-hq/playwright-skill
```
*註：安裝完成後將產生 `.playwright/` 目錄與多份 Markdown 指南供 AI 在需要時自動讀取。*

### AI 模型配置建議
開發測試腳本時，您可以根據不同任務在代理面板切換適合的模型：
- **Gemini 3.1 Pro**：適合用於快速生成最初的基礎測試架構 (Scaffolding) 與探索頁面，速度極快。
- **Claude Sonnet 4.6**：擁用較強的邏輯推理能力，適合用於重構既有代碼或讓代理進行複雜的錯誤修復。
...
## 讓 AI 生成測試：Prompt 實戰技巧

有了良好的配置後，您可以透過兩種主流方式讓 AI 高效生成穩定的測試：

**1. 透過 Playwright MCP (即時讀取頁面)**
依靠 AI 直接開啟瀏覽器擷取 Accessibility Tree。在 Antigravity 對話框中直接輸入 Prompt：
> 「打開 https://storedemo.testdino.com/products，把第一項商品加入購物車，然後前往購物車驗證數量。請使用 Page Object Model 與 getByRole 撰寫這段 Playwright 測試，並使用 Playwright MCP 來輔助探索。」

**2. 結合 CLI Codegen (Token 友善型做法)**
對於非常長或複雜的測試流程，先透過 Codegen 錄下基本步驟，再交給 AI 進行重構：
```powershell
npx playwright codegen https://storedemo.testdino.com
```
接著將產生出的流水帳代碼貼給 AI，並附上 Prompt：
> 「這是剛錄製的測試草稿，請依照我們的 \`.antigravity/rules.md\` 專案規範，將它重構為具有穩健等待機制、重試邏輯以及 POM 架構的正式產線測試腳本。」
...
安裝完成後，務必確認您的 `playwright.config.ts` 內已加入 JSON 與 HTML 報告輸出的支援，TestDino 會需要依賴它們進行分析：
```typescript
